...
add_domain(){
    check_input ${1}
    kubectl exec -it ${CONT_NAME} -- su -s /bin/bash lsadm -c "cd /usr/local/lsws/conf && domainctl.sh --add ${1}"
    if [ ! -d "./sites/${1}" ]; then 
...
del_domain(){
    check_input ${1}
    kubectl exec -it ${CONT_NAME} -- su -s /bin/bash lsadm -c "cd /usr/local/lsws/conf && domainctl.sh --del ${1}"
    bash bin/webadmin.sh -r
}
