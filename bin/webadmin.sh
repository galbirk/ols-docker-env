...
apply_serial(){
    kubectl exec -it  ${CONT_NAME} -- su -c "serialctl.sh --serial ${1}"
    lsws_restart
}

mod_secure(){
    if [ "${1}" = 'enable' ] || [ "${1}" = 'Enable' ]; then
        kubectl exec -it  ${CONT_NAME} -- su -s /bin/bash root -c "owaspctl.sh --enable"
        lsws_restart
    elif [ "${1}" = 'disable' ] || [ "${1}" = 'Disable' ]; then
        kubectl exec -it  ${CONT_NAME} -- su -s /bin/bash root -c "owaspctl.sh --disable"
        lsws_restart
    else
        help_message
    fi
...
ls_upgrade(){
    ...
    kubectl exec -it  ${CONT_NAME} -- su -c '/usr/local/lsws/admin/misc/lsup.sh 2>/dev/null'
}

set_web_admin(){
    echo 'Update web admin password.'
    local LSADPATH='/usr/local/lsws/admin'
    kubectl exec -it  ${CONT_NAME} -- su -s /bin/bash lsadm -c \
